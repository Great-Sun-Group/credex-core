...
export async function GetLedgerController(
  ...
  try {
    ...
    logger.debug("Received get ledger request", {
      requestId,
      accountID,
      numRows,
      startRow,
    ...
    if (!validatePositiveInteger(parsedNumRows)) {
      ...
      return res
        .status(400)
        .json({ error: "Invalid numRows. Must be a positive integer." });
    }
    ...
    if (!Number.isInteger(parsedStartRow) || parsedStartRow < 0) {
      ...
      return res
        .status(400)
        .json({ error: "Invalid startRow. Must be a non-negative integer." });
    }

    logger.debug("Calling GetLedgerService", {
      requestId,
      accountID,
    ...
    logger.error("Unhandled error in GetLedgerController", {
