...
export async function DeclineCredexController(
  ...
  try {
    ...
    logger.debug('Calling DeclineCredexService', { requestId, credexID, signerID });
    ...
    logger.error("Unhandled error in DeclineCredexController", { error: (err as Error).message, stack: (err as Error).stack, requestId });
