...
export async function AcceptCredexBulkController(
  ...
  if (
    ...
    logger.warn("Invalid credexIDs", { requestId, credexIDs: req.body.credexIDs });
  ...
  try {
    logger.debug('Starting bulk accept process', { requestId, credexCount: req.body.credexIDs.length });
    ...
    logger.debug('Filtered valid Credex data', { requestId, validCount: validCredexData.length, totalCount: acceptCredexData.length });
    ...
    if (validCredexData.length > 0) {
      ...
      logger.debug('Fetching dashboard data', { requestId, memberID, acceptorAccountID });
      const dashboardData = await GetAccountDashboardService(memberID, acceptorAccountID);

      if (!dashboardData) {
        logger.error("Failed to fetch dashboard data", { error: "GetAccountDashboardService returned null", requestId, memberID, acceptorAccountID });
        return res.status(500).json({ error: "Failed to fetch dashboard data" });
      }

      logger.info("Credexes accepted in bulk successfully", { count: validCredexData.length, requestId, memberID, acceptorAccountID });
      res.json({
      ...
      logger.warn("No valid data returned from AcceptCredexService", { requestId, credexIDs: req.body.credexIDs });
      res.status(400).json({ error: "No valid data returned from AcceptCredexService" });
    }
  } catch (err) {
    logger.error("Error in AcceptCredexBulkController", { error: (err as Error).message, stack: (err as Error).stack, requestId });
