...
export async function UpdateMemberTierController(
  ...
  logger.debug("Entering UpdateMemberTierController", {
    memberID,
    tier,
    requestId,
  });
  ...
  try {
    ...
    if (result) {
      ...
      logger.debug("Exiting UpdateMemberTierController with success", {
        requestId,
      });
      ...
      logger.debug("Exiting UpdateMemberTierController with failure", {
        requestId,
      });
    ...
    logger.debug("Exiting UpdateMemberTierController with error", {
      requestId,
    });
...
export async function updateMemberTierExpressHandler(
  ...
  logger.debug("Entering updateMemberTierExpressHandler", {
    body: req.body,
    requestId,
  });
  ...
  try {
    ...
    if (!validateUUID(memberID)) {
      ...
      logger.debug(
        "Exiting updateMemberTierExpressHandler with invalid memberID",
        { requestId }
      );
      return;
    }
    ...
    if (!validateTier(tier)) {
      ...
      logger.debug("Exiting updateMemberTierExpressHandler with invalid tier", {
        requestId,
      });
      return;
    }
    ...
    if (result.success) {
      logger.info("Member tier update request successful", {
        memberID,
        tier,
        requestId,
      });
      ...
      logger.warn("Member tier update request failed", {
        memberID,
        tier,
    ...
    logger.debug("Exiting updateMemberTierExpressHandler with result", {
    ...
    logger.debug("Exiting updateMemberTierExpressHandler with error", {
      requestId,
    });
    next(error);
  }
