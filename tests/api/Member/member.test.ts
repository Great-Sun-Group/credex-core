...
describe('Member API Tests', () => {
  ...
  describe('Member Endpoints', () => {
    it('should onboard a new member successfully', async () => {
      ...
      const response = await axios.post('/api/v1/member/onboardMember', memberData);
      ...
      expect(response.data.memberDashboard.accountIDS.length).toBeGreaterThan(0);
    ...
    it('should login a member', async () => {
      ...
      const response = await axios.post('/api/v1/member/login', loginData);
    ...
    it('should get member by handle', async () => {
      ...
      const response = await axios.post('/api/v1/member/getMemberByHandle', data, {
    ...
    it('should get member dashboard by phone', async () => {
      ...
      const response = await axios.post('/api/v1/member/getMemberDashboardByPhone', data, {
        headers: { Authorization: `Bearer ${testMemberJWT}` },
      });
    ...
    it('should authenticate for tier spend limit', async () => {
      ...
      const response = await axios.post('/api/v1/member/authForTierSpendLimit', data, {
        headers: { Authorization: `Bearer ${testMemberJWT}` },
      });
    ...
    it('should set DCO participant rate', async () => {
      ...
      const response = await axios.post('/api/v1/member/setDCOparticipantRate', data, {
        headers: { Authorization: `Bearer ${testMemberJWT}` },
      });
