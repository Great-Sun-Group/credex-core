...
export async function LoginMemberController(
  ...
  try {
    ...
    if (!loginResult.token) {
      logger.warn("Failed to login member", { phone, error: loginResult.error, requestId });
      logger.debug("Exiting LoginMemberController with login failure", { requestId });
...
export async function loginMemberExpressHandler(
  ...
  logger.debug("Entering loginMemberExpressHandler", { body: req.body, requestId });
  ...
  try {
    ...
    if (!validatePhone(phone)) {
      ...
      logger.debug("Exiting loginMemberExpressHandler with invalid phone number", { requestId });
      return;
    }
    ...
    logger.debug("Exiting loginMemberExpressHandler successfully", { requestId });
  } catch (error) {
