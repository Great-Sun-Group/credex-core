...
describe("cancelCredex Endpoint Test", () => {
  ...
  it("cancel credex successful with dashboard data", async () => {
    const params = (process.env.TEST_PARAMS || "").split(" ").filter(Boolean);
    const [token, credexID] = params;

    if (!token || !credexID) {
      throw new Error("Usage: npm test cancelcredex <token> <credexID>");
    }
  ...
  it("not authorized to cancel credex", async () => {
    const params = (process.env.TEST_PARAMS || "").split(" ").filter(Boolean);
    const [token] = params;
