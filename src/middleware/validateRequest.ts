...
function sanitizeAndValidateObject(obj: any, schema: ValidationSchema): { sanitizedObj: any; error: string | null } {
  const sanitizedObj: any = {};
  for (const [key, schemaItem] of Object.entries(schema)) {
    if (typeof schemaItem === 'object' && 'sanitizer' in schemaItem && 'validator' in schemaItem) {
      ...
      if (!validator(sanitizedValue)) {
        logger.debug(`Validation failed for key: ${key}`, { value: sanitizedValue });
      ...
      const { sanitizedObj: nestedSanitizedObj, error: nestedError } = sanitizeAndValidateObject(obj[key], schemaItem as ValidationSchema);
