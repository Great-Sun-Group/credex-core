...
export const GetMemberByHandleController = async (
  ...
  logger.debug("Entering GetMemberByHandleController", { memberHandle, requestId });
  ...
  try {
    if (!validateMemberHandle(memberHandle)) {
      ...
      logger.debug("Exiting GetMemberByHandleController with invalid member handle", { requestId });
      return;
    }
    ...
    if (memberData) {
      logger.info("Member retrieved successfully", { memberHandle, memberID: memberData.memberID, requestId });
    ...
    logger.debug("Exiting GetMemberByHandleController successfully", { requestId });
  } catch (error) {
    ...
    logger.debug("Exiting GetMemberByHandleController with error", { requestId });
    next(error);
  }
};
