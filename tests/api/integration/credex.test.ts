...
// Define error response interface based on API implementation
...
describe("Credex Integration Tests", () => {
  ...
  test("Create and accept secured credex", async () => {
    ...
    testData.credexIDs.secured11USD = createResponse.data.data.action.id;
    ...
    expect(acceptResponse.data.data.action.id).toBeTruthy();
  ...
  test("Create and accept $10 credex, fail $0.01 credex (daily limit)", async () => {
    ...
    expect(create10Response.data.data.action.id).toBeTruthy();
    expect(create10Response.data.data.dashboard).toBeTruthy();
    testData.credexIDs.unsecured10USD = create10Response.data.data.action.id;
  ...
  test("Create multiple credex and test cancel", async () => {
    // Member2 creates $1 credex to member3 (this one will be cancelled)
    ...
    expect(create5Response.data.data.action.id).toBeTruthy();
    expect(create5Response.data.data.dashboard).toBeTruthy();
    testData.credexIDs.unsecured5USD = create5Response.data.data.action.id;
    ...
    expect(create2Response.data.data.action.id).toBeTruthy();
    expect(create2Response.data.data.dashboard).toBeTruthy();
    testData.credexIDs.unsecured2USD = create2Response.data.data.action.id;
    expect(testData.credexIDs.unsecured2USD).toBeTruthy();
    await delay(DELAY_MS * 2);

    // Verify remaining credex have OFFERS status
    ...
    expect(getCredexResponse4.data.data.action.details.transactionType).toBe("OFFERS");
    await delay(DELAY_MS * 2);

    // Member3 accepts remaining credexes in bulk
    ...
    // Verify accepted credex status changed to OWES
    ...
    expect(getCredexResponse6.data.data.action.details.transactionType).toBe("OWES");
    await delay(DELAY_MS * 2);
  });
