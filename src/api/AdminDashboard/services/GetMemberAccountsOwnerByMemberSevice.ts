...
export default async function GetMemberAccountsOwnerByMemberService(memberID: string) {
  ...
  try {
    ...
    logger.info('Accounts owned by member fetched successfully', { memberID, accountCount: accountsOwnedByMember.length });
    logger.debug('GetMemberAccountsOwnerByMemberService exiting successfully', { memberID });
    return {
    ...
    logger.debug('GetMemberAccountsOwnerByMemberService exiting with error', { memberID });
    return {
